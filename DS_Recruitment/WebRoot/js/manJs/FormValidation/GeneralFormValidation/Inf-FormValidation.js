...
jQuery(document).ready(function(){
	...
	$(".speaker").blur(function(){
		...
		logo.css("display","inline");
		if(inputIsEmpty($(this).val())){
			setError(thisbar,logo,mess,"*发布人名字不能为空");
		}else{
			setRight(thisbar,logo,mess);
		}		
	});
	$(".description").blur(function(){
		...
		logo.css("display","inline");
		if(inputIsEmpty($(this).val())){
			setError(thisbar,logo,mess,"*描述不能为空");
		}else{
			setRight(thisbar,logo,mess);
		}		
	});
	$(".pub-inf-btn").click(function(){
		ok = true;
		$.checkAll();
		if(ok){
			$("#pub-info-form").submit();
		}
	});
 ...
 $.extend({
	...
	if(inputIsEmpty($(".speaker").val())){
		ok = false;
		setError($(".speakerBar"),$(".speaker").next(),$(".speaker").next().next(),"*发布人名字不能为空");
	}else{
		setRight($(".speakerBar"),$(".speaker").next(),$(".speaker").next().next());
	}
	
	if(inputIsEmpty($(".description").val())){
		ok = false;
		setError($(".desBar"),$(".description").next(),$(".description").next().next(),"*描述不能为空");
	}else{
		setRight($(".desBar"),$(".description").next(),$(".description").next().next());
	}
